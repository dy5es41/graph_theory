...
class SetUI
{
  ...
  public:
	...
	//	stringRelation: the string relation graph to perform the alogrithm on

	//return: a pair of
	//	int: distance
	//	string: path
	std::pair<int, string> performDijkstra(std::string source, std::string target, StringRelation *sr);
	...
	//clears the console and prints the banner
	void clear();

	//takes input of a flag, and checks if the string relation containing has that property. the fucntion prints if it has that
	//property or not
	//list of flags:
	// "-r" reflexive
	// "-s" symmetric
	// "-t" transitive
	// "-e" equivilance class
	void checkAndPrintFlag(StringRelation *model, std::string flag);
};
